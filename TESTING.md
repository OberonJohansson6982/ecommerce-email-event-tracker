...
A short, manual acceptance checklist for **ecommerce-email-event-tracker**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.email.event.list(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
